...
## 2026-09-07 — the application icon is an AI-generated image

- **Tool / model:** the artwork was generated by ChatGPT (OpenAI); the exact model and prompt
  are the maintainer's and are not recorded here. The maintainer pasted the resulting 2×2
  sheet of four candidates into a Claude Code (Anthropic) session, `claude-opus-5`, which cut
  the sheet apart and wired the chosen candidate in.
- **Scope:** splitting the four-up sheet into four images, and turning the top-left candidate
  into the application icon: `templates/icone/angelith.ico` (16, 24, 32, 48, 64, 128, 256 px),
  its 1024 px master `docs/img/icone-1024.png`, and the three places that name it.
- **Prompt:** "Voici 4 images provenant de ChatGPT, découpe-les en 4 images distinctes et
  prends la première en haut à gauche pour en faire l'icône de l'application."
- **What was changed:** new `templates/icone/angelith.ico` and `docs/img/icone-1024.png`;
  `angelith.spec` gained `icon=` on both executables and ships `templates/icone`;
  `installeur/angelith.iss` gained `SetupIconFile`; `gui/icones.logo()` now loads the shipped
  `.ico` — with all its resolutions — and keeps its inline SVG only as a fallback for a
  missing file; two tests added.
- **What is worth a second look:** **this image is not what `docs/` asked for, and the gap is
  visible.** The icon brief written for this project asks for a flat vector mark, no text, two
  colours plus a neutral, legible at 16 px, and explicitly rules out glow, sparkles, gradients
  and "AI shine". What ships instead is a rendered illustration with a gear, glowing speech
  panels, unreadable page text and a character face. At 48 px and above it reads as an open
  book; at 32 px and 16 px it is a blue smudge with a pale blob, and the taskbar and the
  Alt-Tab list are 16 and 32 px. The choice was the maintainer's, made with the sheet in front
  of them; it is recorded here rather than argued.

  Two consequences that follow from the file rather than from taste: the artwork is a
  raster, so any future size beyond 256 px must be regenerated rather than scaled, which is
  why the 1024 master is committed; and an AGPL binary now carries an image produced by a
  third-party model, whose licensing terms are the generator's, not this project's.
- **Human review:** required. Nothing here was measured — the small-size legibility above is
  an eyeball reading of a rendered strip, not a test.
